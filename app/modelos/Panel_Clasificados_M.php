<?php
    class Panel_Clasificados_M extends ConexionClasificados_BD{
        ...
        //SELECT de todos los productos para  mostrar al usuario, la condicion where es para que muestre un producto una sola vez, si no se coloca muestra un mismo producto tantas imagnes tenga
        ...
        public function consultarImagenesProducto($ID_Producto){
            $stmt = $this->dbh->prepare(
                "SELECT nombre_img 
                FROM imagenes 
                WHERE ID_Producto = :ID_PRODUCTO AND fotoPrincipal = 1"
            );
